...
class EcommerceManager {
    ...
    setupEventListeners() {
        ...
        // Overlay click to close filters
        const filtersOverlay = document.getElementById('filtersOverlay');
        if (filtersOverlay) {
            filtersOverlay.addEventListener('click', () => this.toggleMobileFilters());
        }
        ...
        document.addEventListener('change', (e) => {
            if (e.target.matches('#categoryFilters input[type="checkbox"]')) {
                this.filterProducts();
            }
        });
        ...
        // Checkbox filters
        document.addEventListener('change', (e) => {
            if (e.target.matches('.checkbox-filters input[type="checkbox"]')) {
                this.filterProducts();
            }
        });
    ...
    renderCategories() {
        ...
        this.categories.forEach((products, category) => {
            ...
            categoryItem.className = 'category-filter-item';
    ...
    renderProducts() {
        ...
        if (this.filteredProducts.length === 0) {
            if (this.products.length === 0) {
                ...
                `;
                productsCount.textContent = '0 Artículos';
            } else {
                container.innerHTML = `
                    <div class="empty-state">
                        <h3>Sin Resultados</h3>
        ...
        productsCount.textContent = `${this.filteredProducts.length} Artículos`;
        ...
        const productsHTML = this.filteredProducts.map(product => {
            ...
            // No discount or promotion logic needed
                ...
                <div class="product-card" onclick="ecommerceManager.openProductModal('${product.id}')">
                    <div class="product-image-section">
                        ...
                        <div class="product-actions">
                            <button class="product-action-btn" onclick="event.stopPropagation(); ecommerceManager.openProductModal('${product.id}')">
                    ...
                    <div class="product-content">
                        ...
                        <div class="product-brand">SOFT DUCK</div>
                        <h3 class="product-name">${product.name}</h3>
                        <div class="product-price-section">
                            <div class="product-price">$${parseFloat(individualPrice).toFixed(2)}</div>
                        </div>
    ...
    filterProducts() {
        ...
        const selectedTypes = Array.from(document.querySelectorAll('.checkbox-filters input:checked'))
            .map(input => input.value);
        ...
        this.filteredProducts = this.products.filter(product => {
            ...
            // Price filter
            const matchesPrice = individualPrice >= priceMin && individualPrice <= priceMax;
            
            // Color filter (if product has color info)
            const matchesColor = selectedColors.length === 0 || 
                !product.color || selectedColors.includes(product.color.toLowerCase());
            
            // Product type filter
            const matchesType = selectedTypes.length === 0 || 
                (selectedTypes.includes('individual') && individualPrice > 0) ||
                (selectedTypes.includes('wholesale') && wholesalePrice < individualPrice);
            
            return matchesSearch && matchesCategory && matchesPrice && matchesColor && matchesType;
        });
        ...
        switch (sortValue) {
            ...
            case 'price-asc':
                this.filteredProducts.sort((a, b) => {
                    const priceA = a.price || a.individualPrice || 0;
                    const priceB = b.price || b.individualPrice || 0;
    ...
    clearAllFilters() {
        ...
        document.querySelectorAll('#categoryFilters input[type="checkbox"]').forEach(input => {
            input.checked = false;
        });
        ...
        if (priceMin && priceMax) {
            ...
            document.getElementById('priceMaxDisplay').textContent = '$10,000';
        }
        ...
        document.querySelectorAll('.checkbox-filters input[type="checkbox"]').forEach(input => {
            input.checked = false;
        });
